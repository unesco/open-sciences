...
class FundingOrgFilterBackend(BaseFilterBackend):
    ...
    Note: funder.name is a text field, so we use aggregation or scroll
    to count distinct documents per funding organization.
    """
    ...
    def _normalize_funding_name(self, name: str) -> str:
        """Normalize funding organization name by removing extra spaces."""
        return name.strip()
    ...
    def execute(
        ...
        """
        Override execute for funding - use aggregation for better performance.
        ...
        try:
            ...
            try:
                query_body = {
                    ...
                    "aggs": {
                        "unique_funders": {
                            "terms": {
                                "field": "metadata.funding.funder.name.keyword",
                                "size": 10000,  # Get all unique funding organizations
                                "order": {"_count": "desc"},
                            }
                        }
                    },
            ...
            for bucket in buckets:
                funder_name = bucket["key"]
                doc_count = bucket["doc_count"]

                # Normalize the name
                normalized = self._normalize_funding_name(funder_name)
                ...
                if normalized in normalized_counts:
                    ...
                    normalized_counts[normalized] = doc_count
                    original_names[normalized] = funder_name
    ...
    def _execute_with_scroll(
        ...
        """
        Fallback method using scroll API - counts distinct documents per funding organization.
        """
        try:
            ...
            if "query" in base_query:
                ...
                query_body["query"] = {"exists": {"field": "metadata.funding"}}
            ...
            # Aggregate funding organizations by DOCUMENT (not by occurrence)
            ...
            for hit in hits:
                ...
                # Get unique normalized funding organizations in this document
                funders_in_doc = set()
                for funding_item in funding_list:
                    funder_name = funding_item.get("funder", {}).get("name")
                    if funder_name:
                        normalized = self._normalize_funding_name(funder_name)
                        funders_in_doc.add(normalized)
                        # Track original name
                        if normalized not in original_names:
                            original_names[normalized] = funder_name

                # Add document ID to each normalized funder's set
                for normalized in funders_in_doc:
            ...
            # Count distinct documents per normalized funder
            normalized_counts = {}
